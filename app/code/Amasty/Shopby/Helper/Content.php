...
class Content extends AbstractHelper implements CategoryDataSetterInterface
{
    ...
    private function getAppliedOptionSettings()
    {
        if ($this->optionSettings === null) {
            ...
            foreach ($this->_helper->getSelectedFiltersSettings() as $row) {
                ...
                if ($attributeModel === null || $this->isBrandAndBrandPage($attributeModel)) {
                    continue;
                }
    ...
    private function isBrandAndBrandPage(?Attribute $attributeModel): bool
    {
        $isBrandPageOrAjax = $this->_helper->isBrandPage() || $this->_helper->isShopbyPageWithAjax();
        $isBrandAttribute = $this->baseHelper->getBrandAttributeCode() == $attributeModel->getAttributeCode();

        return $attributeModel && $isBrandPageOrAjax && $isBrandAttribute;
    }
