...
class Rule extends \Magento\Rule\Model\ResourceModel\AbstractResource
{
    ...
    public function getRestrictedProductIds($date, $storeId, $customerGroupId, $customerId = 0)
    {
        ...
        // do sub select for grouping after sorting by priority
        $groupSelect = $this->getConnection()->select();
        $groupSelect->from($select)->group('product_id')->having('hide_product = ?', 1);

        return $this->getConnection()->fetchCol($groupSelect);
    }
    ...
    public function getCategoryIdsFromCollection(\Amasty\Groupcat\Model\ResourceModel\Rule\Collection $collection)
    {
        ...
        $select->reset($select::COLUMNS)
            ...
            ->order('priority ' . $select::SQL_DESC);
        // do sub select for grouping after sorting by priority
        $groupSelect = $this->getConnection()->select();
        $groupSelect->from($select)->group('category_id')->having('hide_category = ?', 1);

        return $connection->fetchCol($groupSelect);
    }
