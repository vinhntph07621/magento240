 ...
 * Time: 17:04
 */
namespace Omnyfy\Vendor\Model\Resource\Vendor\Indexer\Eav;
...
{
    /**
     * Omnyfy resource helper
     *
     * @var \Omnyfy\Vendor\Model\Resource\Helper
     */
     ...
     * @param \Omnyfy\Vendor\Model\Resource\Helper $resourceHelper
    ...
    {
        $this->_init('omnyfy_vendor_vendor_index_eav', 'entity_id');
    }
    ...
    protected function _getIndexableAttributes($multiSelect)
    {
        $select = $this->getConnection()->select()->from(
            ['ca' => $this->getTable('omnyfy_vendor_eav_attribute')],
            'attribute_id'
        )->join(
    ...
    protected function _prepareSelectIndex($entityIds = null, $attributeId = null)
    {
        ...
        $productIdField = $this->getMetadataPool()->getMetadata(VendorInterface::class)->getLinkField();
        ...
        $subSelect = $connection->select()->from(
            ...
            ['dd' => $this->getTable('omnyfy_vendor_vendor_entity_int')],
            'dd.store_id = 0',
            ['attribute_id']
        )->joinLeft(
            ['ds' => $this->getTable('omnyfy_vendor_vendor_entity_int')],
            ...
            ['d2d' => $this->getTable('omnyfy_vendor_vendor_entity_int')],
            sprintf(
                "d2d.store_id = 0 AND d2d.{$productIdField} = dd.{$productIdField} AND d2d.attribute_id = %s",
                $this->_eavConfig->getAttribute(\Omnyfy\Vendor\Model\Vendor::ENTITY, 'status')->getId()
            ),
            []
        )->joinLeft(
            ['d2s' => $this->getTable('omnyfy_vendor_vendor_entity_int')],
            "d2s.store_id = s.store_id AND d2s.attribute_id = d2d.attribute_id AND " .
            "d2s.{$productIdField} = d2d.{$productIdField}",
            []
        )->joinLeft(
            ['cpe' => $this->getTable('omnyfy_vendor_vendor_entity')],
            ...
            (new \Zend_Db_Expr('COALESCE(d2s.value, d2d.value)')) . ' = ' . VendorInterface::STATUS_ENABLED
        )->distinct(true);
        ...
        $select = $connection->select()->distinct(true)->from(
            ...
            ['pis' => $this->getTable('omnyfy_vendor_vendor_entity_int')],
        ...
        $this->_eventManager->dispatch(
            'prepare_omnyfy_vendor_vendor_index_select',
            [
                'select' => $select,
    ...
    protected function _prepareMultiselectIndex($entityIds = null, $attributeId = null)
    {
        ...
        $select = $connection->select()->from(
            ['pvd' => $this->getTable('omnyfy_vendor_vendor_entity_varchar')],
            []
        )->join(
            ...
            ['pvs' => $this->getTable('omnyfy_vendor_vendor_entity_varchar')],
        ...
        $statusCond = $connection->quoteInto('=?', VendorInterface::STATUS_ENABLED);
    ...
    {
        return $this->tableStrategy->getTableName('omnyfy_vendor_vendor_index_eav');
    }
}
