...
class ProcessInvoiceTotal extends Command
{
    protected $appState;
    ...
    {
        $this->setName('omnyfy:vendor:invoice_total');
        $this->setDescription('Process Vendor Invoice Total');
        parent::configure();
    }
    ...
    )
    {
        ...
        while($qItem = $this->queueHelper->takeMsgFromQueue('vendor_invoice_total')) {
            $i++;
            ...
            if (empty($itemData) || !isset($itemData['invoice_id'])) {
                $this->queueHelper->updateQueueMsgStatus($qItem['id'], 'blocking');
                $invalid++;
                continue;
            }
            $invoiceId = $itemData['invoice_id'];

            $result = $this->vendorHelper->calculateVendorInvoiceTotal($invoiceId);
