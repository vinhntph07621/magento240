...
abstract class Attribute extends \Magento\Backend\App\Action
{
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'Omnyfy_Vendor::location_attributes';
    ...
    protected function createActionPage($title = null)
    {
        ...
        if ($this->getRequest()->getParam('popup')) {
            if ($this->getRequest()->getParam('location_tab') === 'variations') {
                $resultPage->addHandle(['popup', 'omnyfy_vendor_attribute_edit_location_tab_variations_popup']);
            } else {
                $resultPage->addHandle(['popup', 'omnyfy_vendor_attribute_edit_location_popup']);
            }
            $pageConfig = $resultPage->getConfig();
            $pageConfig->addBodyClass('attribute-popup');
        } else {
            $resultPage->addBreadcrumb(__('Marketplace'), __('Marketplace'))
                ->addBreadcrumb(__('Manage Location Attributes'), __('Manage Location Attributes'))
                ->setActiveMenu('Omnyfy_Vendor::location_attributes');
            if (!empty($title)) {
