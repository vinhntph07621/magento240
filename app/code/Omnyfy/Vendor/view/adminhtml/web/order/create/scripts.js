...
define([
    ...
    AdminOrder.prototype = {
        ...
        setLocationIds: function(locationIds){
            this.locationIds = locationIds;
        },
        setShippingMethod : function(method,locationId){
            this.shippingMethods[locationId] = method;
            var allSelected=true;
            for(var i=0; i< this.locationIds.length; i++) {
                var lid = this.locationIds[i];
                if (!(lid in this.shippingMethods)) {
                    allSelected = false;
                }
            }
            if (allSelected) {
                var data = {};
                for(lid in this.shippingMethods){
                    data['order[shipping_method][' + lid +']'] = this.shippingMethods[lid];
                }

                //load area if all location got method selected
