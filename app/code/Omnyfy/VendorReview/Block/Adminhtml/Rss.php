...
class Rss extends \Magento\Backend\Block\AbstractBlock implements DataProviderInterface
{
    ...
    /**
     * @var \Magento\Catalog\Model\Rss\Vendor\Review
     */
    protected $rssModel;
     ...
     * @param \Omnyfy\VendorReview\Model\Rss $rssModel
     * @param array $data
     */
    public function __construct(
        \Magento\Backend\Block\Context $context,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Omnyfy\VendorReview\Model\Rss $rssModel,
        array $data = []
    ) {
    ...
    public function getRssData()
    {
        ...
        $title = __('Pending vendor review(s)');

        $data = ['title' => $title, 'description' => $title, 'link' => $newUrl, 'charset' => 'UTF-8'];

        foreach ($this->rssModel->getVendorCollection() as $item) {
            if ($item->getStoreId()) {
                $this->_urlBuilder->setScope($item->getStoreId());
            }

            $url = $this->getUrl('catalog/vendor/view', ['id' => $item->getId()]);
            $reviewUrl = $this->getUrl('vendorreview/vendor/edit/', [
                'id' => $item->getOmnyfyVendorReviewId(),
                '_secure' => true,
                '_nosecret' => true
            ]);

            $storeName = $this->storeManager->getStore($item->getStoreId())->getName();
            $description = '<p>' . __('Vendor: <a href="%1" target="_blank">%2</a> <br/>', $url, $item->getName())
            ...
            $data['entries'][] = [
                'title' => __('Vendor: "%1" reviewed by: %2', $item->getName(), $item->getNickname()),
