...
class Review extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     *
     * @var \Omnyfy\VendorReview\Model\ResourceModel\Rating\Option
     */
     ...
     * @param \Omnyfy\VendorReview\Model\ResourceModel\Rating\Option $ratingOptions
    ...
    {
        $this->_init('vendor_review', 'omnyfy_vendor_review_id');
        ...
        $this->_aggregateTable = $this->getTable('omnyfy_vendor_review_entity_summary');
    }
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        $select->join(
            ...
            $this->getMainTable() . ".omnyfy_vendor_review_id = {$this->_reviewDetailTable}.omnyfy_vendor_review_id"
        );
        return $select;
    }
    ...
    protected function _afterSave(AbstractModel $object)
    {
        ...
        $select = $connection->select()->from($this->_reviewDetailTable, 'detail_id')->where('omnyfy_vendor_review_id = :omnyfy_vendor_review_id');
        $detailId = $connection->fetchOne($select, [':omnyfy_vendor_review_id' => $object->getId()]);
        ...
        if ($detailId) {
            ...
            $detail['omnyfy_vendor_review_id'] = $object->getId();
        ...
        $stores = $object->getStores();
        if (!empty($stores)) {
            $condition = ['omnyfy_vendor_review_id = ?' => $object->getId()];
            ...
            foreach ($stores as $storeId) {
                ...
                $storeInsert = ['store_id' => $storeId, 'omnyfy_vendor_review_id' => $object->getId()];
    ...
    protected function _afterLoad(AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            ...
            'omnyfy_vendor_review_id = :omnyfy_vendor_review_id'
        );
        $stores = $connection->fetchCol($select, [':omnyfy_vendor_review_id' => $object->getId()]);
    ...
    public function getTotalReviews($entityPkValue, $approvedOnly = false, $storeId = 0)
    {
        ...
        if ($storeId > 0) {
            $select->join(
                ...
                $this->_reviewTable . '.omnyfy_vendor_review_id=store.omnyfy_vendor_review_id AND store.store_id = :store_id',
                []
            );
        ...
        if ($approvedOnly) {
            ...
            $bind[':status_id'] = \Omnyfy\VendorReview\Model\Review::STATUS_APPROVED;
        }
    ...
    public function aggregate($object)
    {
        if (!$object->getEntityPkValue() && $object->getId()) {
            $object->load($object->getOmnyfyVendorReviewId());
        }
     ...
     *
     * @param \Magento\Framework\Model\AbstractModel $object
     * @param \Omnyfy\VendorReview\Model\Rating $ratingSummaryObject
     * @return void
     */
    ...
    protected function _loadVotedRatingIds($reviewId)
    {
        ...
        $select = $connection->select()->from(['v' => $this->getTable('vendor_rating_option_vote')], 'r.vendor_rating_id')
            ->joinInner(['r' => $this->getTable('vendor_rating')], 'v.vendor_rating_id=r.vendor_rating_id')
            ->where('v.omnyfy_vendor_review_id = :revire_id');
    ...
    /**
     * Delete reviews by vendor id.
     * Better to call this method in transaction, because operation performed on two separated tables
     *
     * @param int $vendorId
     * @return $this
     */
    public function deleteReviewsByVendorId($vendorId)
    {
        $this->getConnection()->delete(
            $this->_reviewTable,
            [
                'entity_pk_value=?' => $vendorId,
                'entity_id=?' => $this->getEntityIdByCode(\Omnyfy\VendorReview\Model\Review::ENTITY_PRODUCT_CODE)
            ]
        );
        $this->getConnection()->delete(
            $this->getTable('omnyfy_vendor_entity_summary'),
            [
                'entity_pk_value=?' => $vendorId,
                'entity_type=?' => $this->getEntityIdByCode(\Omnyfy\VendorReview\Model\Review::ENTITY_PRODUCT_CODE)
            ]
        );
        return $this;
    }
