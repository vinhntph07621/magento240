...
class VendorBankAccountType extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
    ...
    protected function _construct() {
        $this->_init('omnyfy_mcm_vendor_bank_account_type', 'id');
    }
    ...
    public function insertMultiple($table, $data) {
        ...
        $query = $this->getConnection()->select()->from($table)
                ->where('account_type =?', 'Bank Account (Direct Deposit)');
        $result = $this->getConnection()->fetchRow($query);
        if (empty($result)) {
            return $this->getConnection()->insertMultiple($tableName, $data);
        }
    }
